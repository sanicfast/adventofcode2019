    ...
    def run_program(self):
        while True:
            ...
            if opcode == 1:
                ...
                self.intcode_program[p1] = self.inputs.get()
                self.pointer += 2
            elif opcode == 4:
...
def run_permutations_multithreaded(part):
    max_val = 0
    ...
    for sequence in map(list, permutations(x)):
        ...
        # Create and start threads for each amplifier
        threads = []
        for a in amplifiers:
            t = threading.Thread(target=a.run_program)
            threads.append(t)
            t.start()
        ...
        # Get the last output from the final amplifier
